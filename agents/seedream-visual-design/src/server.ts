...
const manifest = { id: "seedream-visual-design", version: "0.1.0" };

export function createSeedreamVisualDesignServer(service: AgentJobApplication<ImageRequest, GeneratedImage>, images: ImageStore, apiToken?: string, executions?: AgentExecutionAdapter<ImageRequest, GeneratedImage>, artifacts?: ExecutionArtifactStore) {
  const jobsServer = createAgentHttpServer({ manifest, service, apiToken, basePath: "/v1/visual-designs", resultPath: "result", parseRequest: (value) => imageRequestSchema.parse(value) });
