...
import type { SeedreamClient } from "./seedream-client.ts";

export class SeedreamVisualDesignGenerator {
  private readonly client: SeedreamClient;
  private readonly images: ImageStore;

  constructor(client: SeedreamClient, images: ImageStore) {
    this.client = client;
    this.images = images;
  }
  async generate(request: ImageRequest, jobId: string, signal?: AbortSignal): Promise<GeneratedImage> {
    const bytes = await this.client.generate(request.prompt, signal);
