...
const prefix = "GLM_IMAGE_TO_CODE_AGENT";
const config = loadExecutionAdapterConfig({
  environmentPrefix: prefix,
  defaultPort: 8093,
  defaultDataDir: ".data/glm-image-to-code",
});
...
const server = createAgentExecutionServer({
  manifest: { id: "glm-image-to-code", version: "0.1.0" },
  ...runtime,
  apiToken: config.apiToken,
});

server.listen(config.port, "0.0.0.0", () => process.stdout.write(`glm-image-to-code agent listening on :${config.port}\n`));
