...
export function createPlatformRuntime(config: ExecutionAdapterConfig) {
  const artifacts = new FileExecutionArtifactStore(config.dataDir, "glm-image-to-code", config.publicBaseUrl);
  ...
  return {
    ...
    executions: new AgentExecutionAdapter({
      ...
      inputs: new SafeJsonExecutionInputResolver({ userAgent: "GlmImageToCodeAgent/0.1" }),
      artifacts,
      callbacks,
