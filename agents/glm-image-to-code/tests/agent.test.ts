...
test("GLM platform adapter produces an overview without invoking the vision model", async () => {
  const dataDir = await mkdtemp(path.join(os.tmpdir(), "glm-platform-"));
  const runtime = createPlatformRuntime({ port: 8093, dataDir, callbackKeyVersion: "test-v1" });
  const envelope = overviewEnvelope("glm-overview");
