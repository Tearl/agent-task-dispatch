...
import type { ExecutionCallback } from "./types.ts";

export interface CallbackSender {
  send(url: string, callback: ExecutionCallback): Promise<void>;
}

export class HttpCallbackSender implements CallbackSender {
...
export class NoopCallbackSender implements CallbackSender {
