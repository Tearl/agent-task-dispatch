...
test("matching comparison uses authoritative snapshots and exposes degraded and pending states", async () => {
  ...
  assert.match(page, /deletionPending/);
  assert.match(page, /Agent 匹配、预留和链上选择均已关闭/);
  assert.match(page, /view\?\.batch\?\.status === "completed" && batchNeedsRematch\(view\.batch, view\.snapshot\?\.candidates\)/);
  assert.match(page, /if \(view\.batch\?\.status === "completed" && batchNeedsRematch\(view\.batch, view\.snapshot\.candidates\)\) return "重新匹配"/);
  assert.match(page, /概览批次已到期，请先校验并恢复 Agent 终态/);
  assert.match(page, /execution\.status !== "running"/);
  assert.match(page, /batch\.status === "completed"/);
  assert.match(page, /candidates\.every\(\(candidate\) => candidate\.overview\?\.status === "invalid"\)/);
  assert.match(page, /Date\.parse\(batch\.deadline\)/);
  assert.match(page, /AgentMatchConstellation/);
  assert.match(page, /aria-label="任务与候选 Agent 匹配关系"/);
  assert.match(page, /ap-match-link--active/);
  assert.match(page, /aria-pressed=\{active\}/);
