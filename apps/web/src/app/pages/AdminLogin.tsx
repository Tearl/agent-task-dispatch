import { ArrowLeft, ServerCog, ShieldAlert, Wallet } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { useNavigate } from "react-router";

import { CtaButton } from "../components/kit/primitives";
import { authenticateWallet, clientRolesForEngineRoles, revokeSession, type WalletProvider } from "../lib/platform-api";
import { useSession } from "../lib/session";

export default function AdminLogin() {
  const navigate = useNavigate();
  const { connected, authorizedRoles, connect } = useSession();
  const [error, setError] = useState<string | null>(null);
  const [connecting, setConnecting] = useState(false);
  const connectInFlight = useRef(false);

  useEffect(() => {
    if (connected && authorizedRoles.includes("admin")) navigate("/admin", { replace: true });
  ...
  const login = async () => {
    ...
    try {
      ...
      if (!clientRolesForEngineRoles(session.roles).includes("admin")) {
        await revokeSession();
        setError("该钱包没有管理员权限。");
        return;
      }
    ...
    <div className="ap-app-bg relative grid min-h-svh w-full place-items-center overflow-hidden px-5 py-20">
      ...
      <div className="ap-glass-strong relative w-full max-w-[440px] rounded-3xl p-6 sm:p-9">
        ...
        <div className="mt-5 flex items-center gap-3">
          <span className="grid h-12 w-12 place-items-center rounded-xl bg-[rgba(56,189,248,0.15)] text-[var(--ap-info)]"><ServerCog size={22} /></span>
          <div>
            <h1 className="text-[20px] text-white">管理员安全登录</h1>
            <p className="text-[13px] text-[var(--ap-muted)]">钱包签名 · 服务端角色校验</p>
          </div>
        </div>
        ...
        <div className="mt-7 rounded-xl border border-[var(--ap-border)] bg-[rgba(10,18,38,0.5)] p-4 text-[13px] leading-6 text-[var(--ap-text-2)]">
          登录后仍由 Engine 对每次对账查询执行管理员角色校验；浏览器不会直接访问 Engine 或数据库。
        </div>
        ...
        <div className="mt-7">
          <CtaButton full icon={Wallet} disabled={connecting} onClick={() => void login()}>
            {connecting ? "正在验证管理员权限…" : "连接钱包并登录"}
          </CtaButton>
        </div>
        <p className="mt-5 text-center text-[12px] text-[var(--ap-muted)]">仅授权管理员钱包可访问；所有管理操作均应写入只追加审计日志。</p>
      </div>
    </div>
  );
}
