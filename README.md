...
When rotating an immutable escrow deployment, keep each old deployment in
`ESCROW_LEGACY_DEPLOYMENTS_JSON` with its chain, contract, asset key, and
resolver address. The Engine persists that metadata and refuses to start a
legacy projector when the deployment metadata is missing.
...
The local acceptance runner starts PostgreSQL, Redis, LocalStack, and a pinned
Anvil container with Docker Compose, deploys `TaskEscrow`, applies
the Engine migrations, verifies the infrastructure, and runs every required
quality gate:
...
The command requires Docker with Compose, Go, pnpm, Foundry (`forge` and
`cast`), AWS CLI, `jq`, and `curl`. It uses only Anvil's public unlocked
development accounts and never writes a private key. Non-secret chain output is
written to the ignored `.env.chain`; structured integration-test output and the
Go build cache are written below the ignored `.local/acceptance/` directory.

To avoid colliding with an existing development environment, acceptance uses
its own Compose project and host ports: PostgreSQL `55432`, Redis `56379`,
LocalStack `14566`, and Anvil `18545`. The `LOCAL_ACCEPTANCE_*` environment
variables in `scripts/local-acceptance.sh` allow explicit overrides.

The runner fails if any tagged PostgreSQL integration test is skipped. It sets
`ENGINE_TEST_POSTGRES_URL` to the local Compose database, while each test keeps
its existing isolated-schema boundary. Services remain running after a
successful verification so failures can be inspected or development can
continue. The individual lifecycle commands are:

```bash
pnpm acceptance:up
pnpm acceptance:smoke
pnpm acceptance:down
```

`acceptance:up` is idempotent: it waits for all Compose health checks, redeploys
the local contract, and reapplies checksum-protected migrations.
`acceptance:down` stops the isolated Compose project without deleting its named
volumes.
...
## Frozen Escrow V3 testnet decision

Escrow V3 is testnet-only: Anvil `31337` locally and Base Sepolia `84532`
publicly. It binds one six-decimal test USDC asset per immutable deployment,
uses separate 2-of-3 Safe governance and resolver addresses, and upgrades by
deploying a new version rather than a proxy. See
[`docs/adr/0001-escrow-v3-testnet.md`](docs/adr/0001-escrow-v3-testnet.md).
